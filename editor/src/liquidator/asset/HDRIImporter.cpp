...
Result<Path> HDRIImporter::loadFromPath(const Path &originalAssetPath,
  ...
  auto irradianceCubemap = generateIrradianceMap(unfilteredCubemap);
  auto specularCubemap = generateSpecularMap(unfilteredCubemap);
  auto brdfLut = generateBrdfLut(unfilteredCubemap);

  stbi_image_free(data);

  {
    AssetData<TextureAsset> asset{};
    asset.name = originalAssetPath.filename().string() + "-cubemap";
    asset.size =
    ...
    asset.data.height = unfilteredCubemap.levels.at(0).width;
    asset.data.layers = 1;
    asset.data.format = TargetFormat;
    asset.data.levels = unfilteredCubemap.levels;

    asset.data.data = malloc(asset.size);

    TextureUtils::copyTextureToData(mDevice, unfilteredCubemap.texture,
                                    rhi::ImageLayout::ShaderReadOnlyOptimal,
                                    CubemapSides, asset.data.levels,
                                    asset.data.data);
  ...
  {
    AssetData<TextureAsset> asset{};
    asset.name = originalAssetPath.filename().string() + "-irradiance";
    asset.size =
        TextureUtils::getBufferSizeFromLevels(irradianceCubemap.levels);
    asset.data.type = TextureAssetType::Cubemap;
    asset.data.width = irradianceCubemap.levels.at(0).width;
    asset.data.height = irradianceCubemap.levels.at(0).height;
    asset.data.layers = 1;
    asset.data.format = TargetFormat;
    asset.data.levels = irradianceCubemap.levels;

    asset.data.data = malloc(asset.size);

    TextureUtils::copyTextureToData(mDevice, irradianceCubemap.texture,
                                    rhi::ImageLayout::ShaderReadOnlyOptimal,
                                    CubemapSides, asset.data.levels,
                                    asset.data.data);
  ...
  {
    AssetData<TextureAsset> asset{};
    asset.name = originalAssetPath.filename().string() + "-specular";
    asset.size =
        TextureUtils::getBufferSizeFromLevels(unfilteredCubemap.levels);
    asset.data.type = TextureAssetType::Cubemap;
    asset.data.width = specularCubemap.levels.at(0).width;
    asset.data.height = specularCubemap.levels.at(0).height;
    asset.data.layers = 1;
    asset.data.format = TargetFormat;
    asset.data.levels = unfilteredCubemap.levels;
    ...
    TextureUtils::copyTextureToData(mDevice, specularCubemap.texture,
                                    rhi::ImageLayout::ShaderReadOnlyOptimal,
                                    CubemapSides, asset.data.levels,
                                    asset.data.data);
  ...
  {
    AssetData<TextureAsset> asset{};
    asset.name = originalAssetPath.filename().string() + "-brdf";
    asset.size = TextureUtils::getBufferSizeFromLevels(brdfLut.levels);
    asset.data.type = TextureAssetType::Standard;
    asset.data.width = brdfLut.levels.at(0).width;
    asset.data.height = brdfLut.levels.at(0).height;
    asset.data.layers = 1;
    asset.data.format = TargetFormat;
    asset.data.levels = brdfLut.levels;

    asset.data.data = malloc(asset.size);

    TextureUtils::copyTextureToData(
        mDevice, brdfLut.texture, rhi::ImageLayout::ShaderReadOnlyOptimal,
        asset.data.layers, asset.data.levels, asset.data.data);

    auto createdFileRes = mAssetCache.createTextureFromAsset(asset);
  }

  return Result<Path>::Error("Failed to load HDR");
}
...
HDRIImporter::generateIrradianceMap(const CubemapData &unfilteredCubemap) {
  ...
  return CubemapData{irradianceCubemap, {unfilteredCubemap.levels.at(0)}};
}
...
HDRIImporter::generateSpecularMap(const CubemapData &unfilteredCubemap) {
  ...
  return CubemapData{specularCubemap, unfilteredCubemap.levels};
}
...
HDRIImporter::generateBrdfLut(const CubemapData &unfilteredCubemap) {
  ...
  commandList.bindDescriptor(mPipelineGenerateBrdfLut, 0,
                             mDescriptorGenerateCubemap);
  commandList.dispatch(textureDesc.width / 16, textureDesc.height / 16, 1);
  mDevice->submitImmediate(commandList);

  return CubemapData{brdfLut,
                     {{0,
                       static_cast<size_t>(textureDesc.width) *
                           textureDesc.height * FormatSize,
